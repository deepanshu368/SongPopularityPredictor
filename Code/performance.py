...
def perform_visualisation(params, metrics, ax, param_string):
    ...
    sns.lineplot(x=params, y=acc_train, color="b",ax=ax[0])
    sns.lineplot(x=params, y=acc_test, color="r",ax=ax[0])
    ...
    sns.lineplot(x=params, y=f1_train, color="b",ax=ax[1])
    sns.lineplot(x=params, y=f1_test, color="r",ax=ax[1])
...
def plot_roc(roc_dict):
    ...
    for roc in rocs:
        ...
        plt.legend(labels=roc_dict.keys(), loc="lower right")       
        plt.xlim([0.0, 1.0])
        plt.ylim([0.0, 1.05])
